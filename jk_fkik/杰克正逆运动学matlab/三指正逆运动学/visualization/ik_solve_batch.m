%% =========================================================================
%   ILDA 手 MCP 关节逆运动学求解 - 批量求解
%   根据 T_pose_reference.mat 中的 T 点坐标序列，批量求解关节角
%   依赖：params.m, ik_q1_q2_numerical.m, ik_d1_d2.m
% =========================================================================
%
%   输入：T_pose_reference.mat 文件中的 T 点坐标序列
%   输出：all_points - 所有点坐标结构体数组
%         all_info   - 求解信息结构体数组
%
% =========================================================================

function [all_points, all_info] = ik_solve_batch()%数据在函数内部加载
% IK_SOLVE_BATCH 逆运动学批量求解
%   输入：T_pose_reference.mat 文件
...
%         all_info   - 1×N 结构体数组，包含求解信息
    ...
    try
        ...
        fprintf('成功加载 %d 帧 T 点坐标\n', num_frames);
    catch
        error('未找到 T_pose_reference.mat 文件！请先运行 fk_visualize_gif 生成参考文件。');
    end
    ...
    for i = 1:num_frames
        ...
        %% 5.1 求解 q1、q2（同时得到 q3、q4）- 与 ik_solve.m 相同的方法
        ...
        %% 5.3 计算 B1、B2 点（动平台铰链点）- 参考 fk_generate_points.m
        ...
        %% 5.5 计算 M、N 点（三杆机构）- 参考 fk_generate_points.m
