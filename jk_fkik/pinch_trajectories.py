"""
pinch_trajectories.py - 捏合动作轨迹生成
...
_XML_PATH = os.path.join(os.path.dirname(__file__), 'mujoco_model', 'jack_hand.xml')
...
class PalmFrameTransform:
    """Palm 坐标系 与 各手指 FK 坐标系 之间的变换"""

    def __init__(self):
        self._Ry_neg90 = self._Ry(np.radians(-90))

        self._finger_offsets = {
            'index':  np.array([-27.0, 0.0, 68.0]),
            'middle': np.array([-9.0, 0.0, 72.0]),
            'ring':   np.array([9.0, 0.0, 74.0]),
            'little': np.array([27.0, 0.0, 72.0]),
        }
        self._thumb_offset = np.array([-35.0, 5.0, 5.0])
...
def thumb_fk_mjcf(q1_deg, q2_deg, q3_deg, q4_deg, params):
    ...
    输入: MJCF qpos 度数, 输出: tip 在 palm frame 的位置 (mm)"""
    ...
    _thumb_offset = np.array([-35.0, 5.0, 5.0])
...
def finger_fk_mjcf(q1_deg, q2_deg, q3_deg, params):
    ...
    返回 tip 在 finger base frame 中的位置 (mm)"""
    q1 = math.radians(q1_deg)
    q2 = math.radians(q2_deg)
...
class PinchTrajectoryGenerator:
    ...
    def generate_pinch_trajectory(self, finger_name, n_points=50):
        ...
        print(f"  四指 tip: [{search['finger_tip_palm'][0]:.1f}, "
        ...
        print(f"  四指 tip: [{f_tip[0]:.1f}, {f_tip[1]:.1f}, {f_tip[2]:.1f}]")
